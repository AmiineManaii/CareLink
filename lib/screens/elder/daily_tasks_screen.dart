...
class _DailyTasksScreenState extends State<DailyTasksScreen> {
  ...
  Future<void> _scheduleNotification(dynamic task) async {
    ...
    final DateTime warningTime = actualTaskTime.subtract(const Duration(minutes: 15));
    ...
    if (warningTime.isBefore(DateTime.now())) {
      ...
      if (actualTaskTime.isAfter(DateTime.now().subtract(const Duration(seconds: 59)))) {
...
Future<void> _toggleTaskStatus(dynamic task) async {
  ...
  try {
    ...
    if (result['success'] == true) {
      const channel = MethodChannel('fall_channel');
      await channel.invokeMethod('cancelTask', {'id': task['_id'].toString()});
...
IconData _getTaskIcon(String title) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(title: 'Mes Tâches Quotidiennes', showBackButton: true),
